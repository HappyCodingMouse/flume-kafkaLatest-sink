...
import static cxf.flume.sink.kafka.NewKafkaSinkConstants.*;
 ...
 * key
 */
public class NewKafkaSink extends AbstractSink implements Configurable {

  private static final Logger logger = LoggerFactory.getLogger(NewKafkaSink.class);
    ...
    try {
      ...
      for (; processedEvents < batchSize; processedEvents += 1) {
        ...
        if (allowTopicOverride) {
          ...
          if (eventTopic == null) {
            ...
            logger.debug("{} was set to true but header {} was null. Producing to {}" + 
                " topic instead.",
                new Object[]{NewKafkaSinkConstants.ALLOW_TOPIC_OVERRIDE_HEADER,
        ...
        try {
          ...
          kafkaFutures.add(producer.send(record, new NewSinkCallback(startTime)));
  ...
  public void configure(Context context) {
    ...
    useAvroEventFormat = context.getBoolean(NewKafkaSinkConstants.AVRO_EVENT,
                                            NewKafkaSinkConstants.DEFAULT_AVRO_EVENT);

    partitionHeader = context.getString(NewKafkaSinkConstants.PARTITION_HEADER_NAME);
    staticPartitionId = context.getInteger(NewKafkaSinkConstants.STATIC_PARTITION_CONF);

    allowTopicOverride = context.getBoolean(NewKafkaSinkConstants.ALLOW_TOPIC_OVERRIDE_HEADER,
                                          NewKafkaSinkConstants.DEFAULT_ALLOW_TOPIC_OVERRIDE_HEADER);

    topicHeader = context.getString(NewKafkaSinkConstants.TOPIC_OVERRIDE_HEADER,
                                    NewKafkaSinkConstants.DEFAULT_TOPIC_OVERRIDE_HEADER);

    if (logger.isDebugEnabled()) {
      logger.debug(NewKafkaSinkConstants.AVRO_EVENT + " set to: {}", useAvroEventFormat);
    }
  ...
  private void translateOldProps(Context ctx) {
    ...
    if (!(ctx.containsKey(KAFKA_PRODUCER_PREFIX + ProducerConfig.ACKS_CONFIG))) {
      String requiredKey = ctx.getString(
              NewKafkaSinkConstants.REQUIRED_ACKS_FLUME_KEY);
...
class NewSinkCallback implements Callback {
  private static final Logger logger = LoggerFactory.getLogger(NewSinkCallback.class);
  private long startTime;

  public NewSinkCallback(long startTime) {
